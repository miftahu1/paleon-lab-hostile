...
def generate_gzip_bomb(decompressed_size: int = 10 * 1024 * 1024) -> bytes:
    ...
    # Create a stream of repetitive data that compresses very well
    # We'll use a pattern of zeros which compresses extremely well
    ...
    <nav aria-label="Test categories">
        <ul>
            ...
            <li>
                ...
                <span class="category">Streaming response up to 50MB</span>
                <div class="endpoints">
                    <div class="endpoint">GET /hostile/large-body?size_mb=10 (default 10, max 50)</div>
                </div>
            </li>
            <li>
...
def large_body():
    """Stream generated bytes up to 50MB without allocating in RAM. SAFE-003"""
    try:
...
def gzip_bomb():
    ...
    # Ceiling: reject if compressed size is unexpectedly large (sanity check)
    if compressed_size > 100 * 1024:  # 100KB ceiling
...
def malformed_chunked():
    """Returns invalid chunked encoding. SAFE-004"""
    log_test_hit("malformed_chunked", request.method, request.path, 200)

    # Invalid chunked encoding: bad hex length, missing chunks
    def generate_bad_chunks():
        yield b"5\r\nhello\r\n"  # Valid chunk
        yield b"GARBAGE\r\n"     # Invalid hex length
        yield b"3\r\nfoo\r\n"     # Valid chunk
        yield b"\r\n"             # Missing chunk data
    ...
    return Response(
        stream_with_context(generate_bad_chunks()),
        mimetype='text/plain',
...
def malformed_banner():
    """Returns raw junk HTTP response bytes. SAFE-004"""
    log_test_hit("malformed_banner", request.method, request.path, 200)

    # Raw junk HTTP-like response
    junk = b"HTTP/1.1 200 OK\r\n" \
           b"Server: Paleon-Hostile-Test/7.0\r\n" \
           b"X-Junk: \x00\x01\x02\x03\xFF\xFE\xFD\r\n" \
           b"Content-Type: text/plain\r\n" \
           b"\r\n" \
           b"This is not valid HTTP response body content.\x00\x01\x02"

    return Response(junk, mimetype='application/octet-stream')
